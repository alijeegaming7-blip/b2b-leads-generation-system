      ...
      <aside style={{
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"16px", borderBottom:"1px solid rgba(255,255,255,.06)", height:56 }}>
          ...
          <span style={{
            ...
            border:"1px solid rgba(99,102,241,.3)", borderRadius:6, padding:"2px 6px",
          }}>v2</span>
        </div>
        ...
        <nav style={{ flex:1, padding:"8px 12px", overflowY:"auto" }}>
          ...
          {NAV.map(item => {
            if ((item as any).disabled) return (
              <div key={item.to + item.label} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 10px", borderRadius:8, color:"rgba(255,255,255,.2)", fontSize:13, cursor:"not-allowed", marginBottom:2 }}>
            ...
            return (
              <NavLink key={item.to} to={item.to} style={{ display:"block", marginBottom:2, textDecoration:"none" }}>
                <div style={{
                  ...
                  color: isActive ? "#a5b4fc" : "rgba(255,255,255,.6)",
                  fontSize:13, fontWeight: isActive ? 600 : 400,
                  transition:"all 0.15s",
                }}
                  onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,.05)"; (e.currentTarget as HTMLElement).style.color = "#fff"; }}
                ...
                >
                  ...
                  <span style={{ flex:1 }}>{item.label}</span>
                  {(item as any).badge && (
                    <span style={{ fontSize:9, fontWeight:700, background:"rgba(99,102,241,.25)", color:"#a5b4fc", border:"1px solid rgba(99,102,241,.3)", borderRadius:6, padding:"1px 5px" }}>
                      {(item as any).badge}
                    </span>
                  )}
